...
export default class TuiCalendar extends React.Component  {
  ...
  onClickSchedule(scheduleData) {
    console.log("onclick schedule")
    const { id } = scheduleData.schedule;
    const el = this.useRef.current.calendarInst.getElement(id);
  ...
  updateScheduleTask(ScheduleData) {
    ...
    const updatedSchedules = schedules.map((schedule) => {
      return schedule.id === schedule.id ? { ...schedule, ...changes } : schedule
    })
    this.UPDATE_API_ROUTE = "/schedule/update";
      axios.patch(this.UPDATE_API_ROUTE, updatedSchedules)
        .then(
          this.setState({
